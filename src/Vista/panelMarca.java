...
import Datos.Marca;
import Datos.Modelo_Tabla_Marca;
...
public class panelMarca extends javax.swing.JPanel {
    ...
    private Modelo_Tabla_Marca MTM = new Modelo_Tabla_Marca();
    ...
    private void initComponents() {
        ...
        jPanel5.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "MARCAS REGISTRADAS", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 14))); // NOI18N
        ...
        tablaMarca.setModel(this.MTM);
        tablaMarca.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        tablaMarca.getTableHeader().setResizingAllowed(false);
    ...
    private void activarControles(boolean estado) {
        ...
        this.tablaMarca.setEnabled(!estado);
        this.btnEliminar.setEnabled(!estado);

        if (estado) {
            tablaMarca.setBackground(new java.awt.Color(255, 102, 102));
        } else {
            tablaMarca.setBackground(new java.awt.Color(255, 255, 255));
        }
    }
    ...
    private void cargarTabla() {
        try {
            ArrayList<Marca> ltsMarca = MarcaBo.obtenerListaMarca();
            MTM.setListado_Marca(ltsMarca);
        } catch (Exception e) {
    ...
    private void definir_tamaño_columna() {
        TableColumnModel columnModel = this.tablaMarca.getColumnModel();
        columnModel.getColumn(0).setPreferredWidth(250);
        columnModel.getColumn(1).setPreferredWidth(250);

        this.tablaMarca.getTableHeader().setFont(new Font("Tahoma", 1, 12));
        ((DefaultTableCellRenderer) this.tablaMarca.getTableHeader().getDefaultRenderer()).setHorizontalAlignment(SwingConstants.CENTER);
    }

    private void guardar() {
        if (this.general()) {
            Marca objMar = new Marca(0, this.txtNombre.getText(), this.estado);
            try {
                if (this.objM == null) {
                    if (MarcaBo.existeMarca(this.txtNombre.getText())) {
                        if (MarcaBo.grabarMarca(objMar)) {
                            JOptionPane.showMessageDialog(this, "Se Registró Correctamente", "MENSAJE --> MARCA", JOptionPane.INFORMATION_MESSAGE);
                        } else {
                            JOptionPane.showMessageDialog(this, "No se pudo Registrar", "MENSAJE --> MARCA", JOptionPane.ERROR_MESSAGE);
                        }
                        this.limpiar();
                        this.activarControles(false);
                        this.cargarTabla();
                    } else {
                        JOptionPane.showMessageDialog(this, "El Nombre ya se encuentra registrado.", "MENSAJE --> MARCA", JOptionPane.WARNING_MESSAGE);
                    }
                } else {
                    boolean existenombre = true;
                    objMar.setId(this.objM.getId());

                    if (!objMar.getNombre().equals(this.objM.getNombre())) {
                        existenombre = MarcaBo.existeMarca(objMar.getNombre());
                    }

                    if (existenombre) {
                        if (MarcaBo.modificarMarca(objMar)) {
                            JOptionPane.showMessageDialog(this, "Se Modificó Correctamente", "MENSAJE --> MARCA", JOptionPane.INFORMATION_MESSAGE);
                        } else {
                            JOptionPane.showMessageDialog(this, "No se pudo Modificar", "MENSAJE --> MARCA", JOptionPane.ERROR_MESSAGE);
                        }
                        this.limpiar();
                        this.activarControles(false);
                        this.cargarTabla();
            ...
            JOptionPane.showMessageDialog(this, "Verifique que todos los campos hallan sido llenados.", "MENSAJE --> MARCA", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private void buscar() {
        ...
        try {
            ltsMarca = MarcaBo.buscarNombreMarca(buscar);
            MTM.setListado_Marca(ltsMarca);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    private void eliminar() {
        if (this.tablaMarca.getSelectedRow() != -1) {
            int fila = this.tablaMarca.getSelectedRow();
            Marca objMar = MTM.getMarca(fila);
            if (objMar.getEstado() == 1) {
                int confirm = JOptionPane.showConfirmDialog(this, "¿Está seguro que desea eliminar la marca "
                        + objMar.toString() + " ?", "ELIMINAR --> MARCA", JOptionPane.YES_NO_OPTION);
                if (confirm == 0) {
                    try {
                        if (MarcaBo.eliminarMarca(objMar)) {
                            this.cargarTabla();
                            this.limpiar();
                            this.activarControles(false);
                        } else {
                            JOptionPane.showMessageDialog(this, "No se pudo Eliminar", "MENSAJE --> MARCA", JOptionPane.ERROR_MESSAGE);
                        }
                    } catch (Exception e) {
                        System.out.println(e.getMessage());
                    }
                ...
                JOptionPane.showMessageDialog(this, "La marca selecionada ya se encuentra Inactiva.", "MENSAJE --> MARCA", JOptionPane.ERROR_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this, "Usted no a seleccionado un campo en la tabla.", "MENSAJE --> MARCA", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void cargarDatos(Marca objM) {
        this.txtNombre.setText(objM.getNombre());
